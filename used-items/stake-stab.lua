-- Stabbed through the vampire to kill it
Stake_Stab = Class {
	init = function(self, originX, originY, dirX, dirY, image)
		...
		local insideVector = Vector(originX, originY) + offsetVector;
			local outsideVector = insideVector + (dirVector * STAKE_RANGE) + offsetVector;

		-- Get outside points of the stake bounded box
		...
		if w < STAKE_MIN_SIZE then
			local diffX = STAKE_MIN_SIZE - w;
			w = STAKE_MIN_SIZE;
			x1 = x1 - diffX / 2;
		end

		if h < STAKE_MIN_SIZE then
			local diffY = STAKE_MIN_SIZE - h;
			h = STAKE_MIN_SIZE;
			y1 = y1 - diffY / 2;
		end
		...
		self.type = "placed-stake-stab";
		self.active = true;
	end
}

function Stake_Stab:update(dt)
	...
	for i = 1, len do
		local col = cols[i];
		if col.other.monsterType == "vampire" then
